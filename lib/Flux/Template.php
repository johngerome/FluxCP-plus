...
class Flux_Template {
	...
	public function render(array $dataArr = array())
	{
		...
		if (file_exists($this->headerPath) && ($this->isAjax === false)) {
			include $this->headerPath;
		}

		// Don't include subheader on installer theme
    if ($this->moduleName != 'install') {
		...
		if (file_exists($this->footerPath) && ($this->isAjax === false)) {
	...
	public function redirect($location = null)
	{
		...
		if ($this->isAjax === true) {
			header("Location: $location&isAjax=true");
		}else{
	...
	{
		$dispatcher = Flux_Dispatcher::getInstance();
		$dispatcher->loginRequired($this->isAjax, $this->basePath, $message);
	}
